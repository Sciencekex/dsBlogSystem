...
你可以在[这里](src/main/java/io/github/sciencekex/model)
查看所有的模型。着重查看 [`User`](src/main/java/io/github/sciencekex/model/User.java) 模型。
...
，写入[这里面](src/main/java/io/github/sciencekex/listener/HibernateListener.java)的内容。
...
详细的仓库层设计你可以看[这里](src/main/java/io/github/sciencekex/repository)
。着重查看 [`UserRepository`](src/main/java/io/github/sciencekex/repository/UserRepository.java) 的设计，其他都是触类旁通。
...
你可以在[这里](src/main/java/io/github/sciencekex/security/AuthenticationFilter.java)查看过滤器 `AuthenticationFilter`
的实现。

并且我们还需要一个 `@Secured` 注解（注意是注解，不是注释，注解是可编程的），用来注解在需要鉴权的控制器上（在讲控制器之前，你可以把控制器理解为请求进来时经过的方法）。你可以在[这里](src/main/java/io/github/sciencekex/security/Secured.java)查看 `@Secured` 注解的定义，定义其实没什么实际含义，真正的逻辑已经写在 `AuthenticationFilter` 的 `extractRoles` 方法中了。
...
，你可以在[这里](src/main/java/io/github/sciencekex/handler)查看所有的 `handler`。
...
具体的解释，请务必到[源码](src/main/java/io/github/sciencekex/handler/UserHandler.java)中查看，有代码才能方便理解！
...
点击 [这里](src/main/java/io/github/sciencekex/handler) 查看后端具体实现
